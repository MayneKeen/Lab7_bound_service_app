...
/*
    ...
    private suspend fun handleWork(receivedIntent: Intent) {
        ...
        if (url != null) {
            ...
            if (path != "") {
                Intent().also { intent ->
                    intent.action = "com.example.lab7_bound_service_app.IMAGE_DOWNLOAD_COMPLETE"
  ...
  /*  private fun downloadImageAsync(url: String): Deferred<Bitmap?> =
    ...
    class IncomingHandler : Handler() {
        ...
        override fun handleMessage(msg: Message) {
            ...
            Log.i(
                "InclomingHandle",
                "got msg with object = $url and what = ${msg.what}"
            )
            ...
            if (msg.what == MSG_REQUEST_PATH) {
                coroutineScope.launch(Dispatchers.IO) {
                    if (url != null) {
                        ...
                        val originalBitmap =
                            downloadImageAsync(url).await()
                        val cw = ContextWrapper(service.applicationContext)
                        val directory = cw.getDir("imageDir", Context.MODE_PRIVATE)
                        var file =
                            File(directory, "downloadedimg" + Random.nextInt(0, 10000) + ".jpg")
                        while (file.exists()) {
                            file =
                                File(directory, "downloadedimg" + Random.nextInt(0, 10000) + ".jpg")
                        }
                        path = file.toString()
                        Log.i("InclomingHandle", "Path is $path")
                        val fos: FileOutputStream?
                        try {
                            fos = FileOutputStream(file)
                            originalBitmap?.compress(Bitmap.CompressFormat.JPEG, 100, fos)
                            ...
                            Log.i("InclomingHandle", "error occurred")
                            e.printStackTrace()
                        }
                        if (path != "") {
                            val msgWithPath = Message.obtain(null, MSG_REQUEST_PATH_RESPONSE)
                            val b = Bundle()
                            b.putString("PATH", path)
                            msgWithPath.data = b
                            replyTo.send(msgWithPath)
                        }
                    }
                }
            } else {
    ...
    companion object {
        private lateinit var service: DownloadImageService
        const val MSG_REQUEST_PATH = 1
        const val MSG_REQUEST_PATH_RESPONSE = 10
    }
}